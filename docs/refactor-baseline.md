...
- `cargo xtask structure`
  ...
  - structure and ownership checks for shared dispatch/helper boundaries; raw function length is intentionally not a gate because line count alone does not define the next engineering action
...
- `bash ./scripts/check-ci-acceptance-gates.sh`
  ...
  - Phase 3 dispatch access, pool-shaping, streaming-limit, DATAGRAM, mirror, decision_service-response, secure-file, and TLS-boundary gates, to keep shared decision_service/audit helpers, H2 reservations, H3 request-open actors, QPX H3 inflight-aware pooling, H3 fail-closed request limits, prefix-aware DATAGRAM send paths, lock-free DATAGRAM receive snapshots, bounded mirror fan-out, cancellable bounded decision_service response collection, hardlink-resistant secure writes, and qpx-core TLS primitives from regressing
...
LOC budgets are advisory trend signals. `xtask` reports LOC drift as a notice, not a failure, so reviewers can ask why code grew without forcing arbitrary shortening. The next action is deliberately explicit: either reduce the drift by removing accidental duplication or update the advisory baseline in `xtask/src/budget.rs` / `xtask/loc-budgets.tsv` with a review rationale when the larger shape is intentional and improves correctness, coverage, or ownership. Do not contort code, delete useful behavior, or split modules only to satisfy a line-count target; prefer clear ownership, lower duplication, better tests, and measurable hot-path improvements. Function length is not separately baseline-gated; use the structural gates below when ownership, duplication, or hot-path resource behavior is the actual concern.
...
- Phase 1: production panic/unwrap and unsafe documentation gates are represented by `xtask`, workspace lints, and clippy/doc jobs. `cargo xtask structure` also verifies the workspace lint posture: rust-version 1.96, `dead_code`/`unused`/`unsafe_op_in_unsafe_fn = "deny"`, and `clippy::undocumented_unsafe_blocks = "deny"`. Public `anyhow::Result` library boundaries are now capped at 0 by `xtask`; new library public APIs must use crate-specific or otherwise explicit result types.
...
- Phase 3: forward, reverse, transparent, and MITM dispatch access paths must keep shared audit/decision_service helpers. Forward request guard/policy audit, MITM guard/decision_service audit, and transparent guard audit also use `build_dispatch_audit_context` instead of reconstructing log context and `DispatchAuditContext` by hand. MITM decision_service continue/deny handling now uses `apply_decision_service_http_access`, matching the HTTP dispatch paths instead of branching by `DecisionServiceEnforcement` locally. Reverse dispatch carries the context built by access control instead of rebuilding it from log-context fragments. `cargo xtask structure` rejects direct `DispatchAuditContext::new`, direct decision_service policy extraction, and direct `DecisionServiceEnforcement` branching in those access files, and rejects direct audit context construction in the representative forward/reverse/transparent/MITM dispatch paths.
...
- Phase 3: reverse mirrors are bounded per endpoint before spawning mirror tasks, and decision_service responses use cancellable bounded async collection; `cargo xtask structure` rejects mirror task spawns that occur before permit acquisition, rejects decision_service response collection falling back to `to_bytes_limited` or `collect().await`, and rejects non-cancellable blocking parsers such as `spawn_blocking` / `SyncIoBridge`, while the acceptance gate keeps both hot-path resource controls present.
...
- Phase 3: reverse decision_service shared-client entrypoints were reduced to direct request functions; compatibility-style singleton client facades should not be reintroduced.
