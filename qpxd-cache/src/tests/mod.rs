...
async fn store_and_drain(
    ...
    let body = qpx_http::body::to_bytes(body).await?;
...
/// Fresh per-test background-revalidation registry (formerly a process-global).
fn test_revalidations() -> Arc<crate::InFlightRevalidations> {
    Arc::new(crate::InFlightRevalidations::with_default_shards())
}
