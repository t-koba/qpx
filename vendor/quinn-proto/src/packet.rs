...
impl LongHeaderType {
    fn from_byte(b: u8, version: u32) -> Result<Self, PacketDecodeError> {
        ...
        let packet_type = (b & 0x30) >> 4;
        Ok(match (version == 0x6b33_43cf, packet_type) {
            (false, 0x0) | (true, 0x1) => Initial,
            (false, 0x1) | (true, 0x2) => Standard(ZeroRtt),
            (false, 0x2) | (true, 0x3) => Standard(Handshake),
            (false, 0x3) | (true, 0x0) => Retry,
            _ => unreachable!(),
        })
    }
    ...
    fn to_byte(self, version: u32) -> u8 {
        use {LongHeaderType::*, LongType::*};
        let packet_type = match (version == 0x6b33_43cf, self) {
            (false, Initial) | (true, Retry) => 0x0,
            (false, Standard(ZeroRtt)) | (true, Initial) => 0x1,
            (false, Standard(Handshake)) | (true, Standard(ZeroRtt)) => 0x2,
            (false, Retry) | (true, Standard(Handshake)) => 0x3,
        };
        LONG_HEADER_FORM | FIXED_BIT | (packet_type << 4)
    }
}
...
mod tests {
    ...
    #[test]
    fn quic_v2_long_header_packet_type_mapping() {
        const V2: u32 = 0x6b33_43cf;
        for (ty, bits) in [
            (LongHeaderType::Retry, 0x00),
            (LongHeaderType::Initial, 0x10),
            (LongHeaderType::Standard(LongType::ZeroRtt), 0x20),
            (LongHeaderType::Standard(LongType::Handshake), 0x30),
        ] {
            let encoded = ty.to_byte(V2);
            assert_eq!(encoded & 0x30, bits);
            assert_eq!(LongHeaderType::from_byte(encoded, V2).unwrap(), ty);
        }
    }
