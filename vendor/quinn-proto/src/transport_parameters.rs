...
macro_rules! make_struct {
    {$($(#[$doc:meta])* $name:ident ($id:ident) = $default:expr,)*} => {
        ...
        pub struct TransportParameters {
            ...
            /// RFC 9368 compatible version negotiation information.
...
impl TransportParameters {
    pub(crate) fn new(
        ...
        Self {
            ...
            version_information: Some(VersionInformation::new(version, supported_versions)),
        ...
        while r.has_remaining() {
            ...
            match id {
                ...
                TransportParameterId::VersionInformation => {
                    ...
                    params.version_information = Some(VersionInformation::read(&mut r.take(len))?);
                }
                _ => {
                    macro_rules! parse {
                        {$($(#[$doc:meta])* $name:ident ($id:ident) = $default:expr,)*} => {
...
const MAX_COMPATIBLE_VERSIONS: usize = 8;

#[derive(Debug, Copy, Clone, Eq, PartialEq)]
pub(crate) struct VersionInformation {
    pub(crate) chosen: u32,
    available: [u32; MAX_COMPATIBLE_VERSIONS],
    available_len: u8,
}

impl VersionInformation {
    fn new(chosen: u32, supported: &[u32]) -> Self {
        let mut available = [0; MAX_COMPATIBLE_VERSIONS];
        let mut available_len = 0;
        for &version in supported.iter().take(MAX_COMPATIBLE_VERSIONS) {
            if available[..available_len].contains(&version) {
                continue;
            }
            available[available_len] = version;
            available_len += 1;
        }
        Self {
            chosen,
            available,
            available_len: available_len as u8,
        }
    }

    fn wire_size(self) -> usize {
        4 + usize::from(self.available_len) * 4
    }

    fn write(self, out: &mut impl BufMut) {
        out.put_u32(self.chosen);
        for &version in &self.available[..usize::from(self.available_len)] {
            out.put_u32(version);
        }
    }

    fn read(input: &mut impl Buf) -> Result<Self, Error> {
        if input.remaining() < 8 || input.remaining() % 4 != 0 {
            return Err(Error::Malformed);
        }
        let chosen = input.get_u32();
        let count = input.remaining() / 4;
        if count > MAX_COMPATIBLE_VERSIONS {
            return Err(Error::Malformed);
        }
        let mut available = [0; MAX_COMPATIBLE_VERSIONS];
        for slot in &mut available[..count] {
            *slot = input.get_u32();
        }
        if !available[..count].contains(&chosen) {
            return Err(Error::IllegalValue);
        }
        Ok(Self {
            chosen,
            available,
            available_len: count as u8,
        })
    }
}
...
mod test {
    ...
    #[test]
    fn rfc9368_version_information_round_trip() {
        let info = VersionInformation::new(0x6b33_43cf, &[1, 0x6b33_43cf]);
        let mut encoded = Vec::new();
        info.write(&mut encoded);
        assert_eq!(encoded.len(), 12);
        assert_eq!(
            VersionInformation::read(&mut encoded.as_slice()).unwrap(),
            info
        );
    }

    #[test]
    fn rfc9368_version_information_requires_chosen_version_in_available_list() {
        let mut encoded = [0x6b, 0x33, 0x43, 0xcf, 0, 0, 0, 1].as_slice();
        assert_eq!(VersionInformation::read(&mut encoded), Err(Error::IllegalValue));
    }
