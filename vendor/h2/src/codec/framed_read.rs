...
fn decode_frame(
    ...
    let span = h2_trace_span!("FramedRead::decode_frame", offset = bytes.len());
    let _e = h2_span_enter!(span);

    h2_trace!("decoding frame from {}B", bytes.len());
    ...
    macro_rules! header_block {
        ($frame:ident, $head:ident, $bytes:ident) => ({
            ...
            if is_end_headers {
                ...
                h2_trace!("loaded partial header block");
        ...
        Kind::Continuation => {
            ...
            if is_end_headers {
                ...
                if cnt > max_continuation_frames {
                    h2_wire_debug!("too_many_continuations, max = {}", max_continuation_frames);
    ...
    T: AsyncRead + Unpin,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let span = h2_trace_span!("FramedRead::poll_next");
        let _e = h2_span_enter!(span);
        loop {
            h2_trace!("poll");
