        ...
        loop {
            while !self.encoder.is_empty() {
                ...
                if n == 0 {
                    ...
                    h2_trace!("write returned zero, but non-zero bytes remaining");
    ...
    B: Buf,
{
    ...
    fn buffer(&mut self, item: Frame<B>) -> Result<(), UserError> {
        ...
        let span = h2_trace_span!("FramedWrite::buffer", frame = ?item);
        let _e = h2_span_enter!(span);

        h2_wire_debug!(frame = ?item, "send");
        ...
        match item {
            ...
            Frame::Settings(v) => {
                ...
                h2_trace!(rem = self.buf.remaining(), "encoded settings");
            }
            Frame::GoAway(v) => {
                v.encode(self.buf.get_mut());
                h2_trace!(rem = self.buf.remaining(), "encoded go_away");
            }
            Frame::Ping(v) => {
                v.encode(self.buf.get_mut());
                h2_trace!(rem = self.buf.remaining(), "encoded ping");
            }
            Frame::WindowUpdate(v) => {
                v.encode(self.buf.get_mut());
                h2_trace!(rem = self.buf.remaining(), "encoded window_update");
            }

            Frame::Priority(_) => {
                /*
                v.encode(self.buf.get_mut());
                h2_trace!("encoded priority; rem={:?}", self.buf.remaining());
                */
                unimplemented!();
            }
            Frame::Reset(v) => {
                v.encode(self.buf.get_mut());
                h2_trace!(rem = self.buf.remaining(), "encoded reset");
            }
        }
