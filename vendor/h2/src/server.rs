    ...
    B: Buf,
{
    fn handshake2(io: T, builder: Builder) -> Handshake<T, B> {
        let span = h2_trace_span!("server_handshake");
        let _entered = h2_span_enter!(span);
        ...
        let state = Handshaking::Flushing(Flush::new(codec).instrument(h2_trace_span!("flush")));
            ...
            match &mut self.state {
                Handshaking::Flushing(flush) => {
                    ...
                    let codec = match Pin::new(flush).poll(cx)? {
                        Poll::Pending => {
                            h2_trace!(flush.poll = %"Pending");
                            return Poll::Pending;
                        }
                        Poll::Ready(flushed) => {
                            h2_trace!(flush.poll = %"Ready");
                            flushed
                        }
                    };
                    self.state = Handshaking::ReadingPreface(
                        ReadPreface::new(codec).instrument(h2_trace_span!("read_preface")),
                    );
                }
                Handshaking::ReadingPreface(read) => {
                    ...
                    h2_trace!("connection established!");
