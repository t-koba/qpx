...
impl Decoder {
        ...
        F: FnMut(Header) -> ControlFlow<()>,
    {
        ...
        let span = h2_trace_span!("hpack::decode");
        ...
        while let Some(ty) = peek_u8(src) {
            ...
            match Representation::load(ty)? {
                Indexed => {
                    h2_trace!(rem = src.remaining(), kind = %"Indexed");
                    can_resize = false;
                ...
                LiteralWithIndexing => {
                    h2_trace!(rem = src.remaining(), kind = %"LiteralWithIndexing");
                    can_resize = false;
                ...
                LiteralWithoutIndexing => {
                    h2_trace!(rem = src.remaining(), kind = %"LiteralWithoutIndexing");
                    can_resize = false;
                ...
                LiteralNeverIndexed => {
                    h2_trace!(rem = src.remaining(), kind = %"LiteralNeverIndexed");
                    can_resize = false;
                    let entry = self.decode_literal(src, false)?;
                    consume(src);
                ...
                SizeUpdate => {
                    h2_trace!(rem = src.remaining(), kind = %"SizeUpdate");
                    if !can_resize {
    ...
    fn try_decode_string(
        ...
        if len > buf.remaining() {
            h2_trace!(len, remaining = buf.remaining(), "decode_string underflow",);
