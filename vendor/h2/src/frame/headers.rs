...
impl Headers {
    ...
    pub fn load(head: Head, mut src: BytesMut) -> Result<(Self, BytesMut), Error> {
        ...
        h2_trace!("loading headers; flags={:?}", flags);
    ...
    fn load(
        ...
        macro_rules! check_size {
            () => {{
                if headers_size > max_header_list_abuse_size {
                    h2_trace!("load_hpack; header list size over abuse max");
                    header_list_way_too_large = true;
                    ControlFlow::Break(())
                } else {
                    if headers_size >= max_header_list_size && !self.is_over_size {
                        h2_trace!("load_hpack; header list size over max");
        ...
        macro_rules! set_pseudo {
            ($field:ident, $val:expr) => {{
                if reg {
                    h2_trace!("load_hpack; header malformed -- pseudo not at head of block");
                    malformed = true;
                } else if self.pseudo.$field.is_some() {
                    h2_trace!("load_hpack; header malformed -- repeated pseudo");
                    malformed = true;
                } else {
                    let __val = $val;
                    headers_size +=
        ...
        let res = decoder.decode(&mut cursor, |header| {
            ...
            match header {
                Field { name, value } => {
                    ...
                    {
                        h2_trace!("load_hpack; connection level header");
                        malformed = true;
                    } else if name == header::TE && value != "trailers" {
                        h2_trace!("load_hpack; TE header not set to trailers; val={:?}", value);
                        malformed = true;
                    } else {
                        reg = true;
        ...
        match res {
            Ok(()) => {}
            Err(e) => {
                h2_trace!("hpack decoding error; err={:?}", e);
                return Err(e.into());
            }
        }

        if header_list_way_too_large {
            h2_trace!("header list way too large; aborting connection");
            return Err(Error::HeaderListWayTooLarge);
        }

        if malformed {
            h2_trace!("malformed message");
            return Err(Error::MalformedMessage);
        }
