    ...
    B: Buf,
{
    pub fn new(codec: Codec<T, Prioritized<B>>, config: Config) -> Connection<T, P, B> {
        ...
        #[cfg(not(feature = "wire-tracing"))]
        let _ = P::NAME;
        #[cfg(feature = "wire-tracing")]
        let span = h2_debug_span!(parent: None, "Connection", peer = %P::NAME);
        #[cfg(feature = "wire-tracing")]
    ...
    fn poll_ready(&mut self, cx: &mut Context) -> Poll<Result<(), Error>> {
        ...
        let span = h2_trace_span!("poll_ready");
    ...
    pub fn poll(&mut self, cx: &mut Context) -> Poll<Result<(), Error>> {
        ...
        #[cfg(feature = "wire-tracing")]
        let _e = h2_span_enter!(span);
        let span = h2_trace_span!("poll");
        let _e = h2_span_enter!(span);

        loop {
            h2_trace!(connection.state = ?self.inner.state);
            ...
            match self.inner.state {
                ...
                State::Closing(reason, initiator) => {
                    h2_trace!("connection closing after flush");
    ...
    B: Buf,
{
        ...
        match result {
            ...
            Err(Error::Reset(id, reason, initiator)) => {
                if initiator == Initiator::Remote {
                    h2_trace!(?id, ?reason, ?initiator, "stream reset");
                    return Ok(());
                }

                debug_assert_eq!(initiator, Initiator::Library);
                h2_trace!(?id, ?reason, ?initiator, "stream error");
    ...
    fn recv_frame(&mut self, frame: Option<Frame>) -> Result<ReceivedFrame, Error> {
        ...
        match frame {
            Some(Headers(frame)) => {
                h2_trace!(?frame, "recv HEADERS");
                self.streams.recv_headers(frame)?;
            }
            Some(Data(frame)) => {
                h2_trace!(?frame, "recv DATA");
                self.streams.recv_data(frame)?;
            }
            Some(Reset(frame)) => {
                h2_trace!(?frame, "recv RST_STREAM");
                self.streams.recv_reset(frame)?;
            }
            Some(PushPromise(frame)) => {
                h2_trace!(?frame, "recv PUSH_PROMISE");
                self.streams.recv_push_promise(frame)?;
            }
            Some(Settings(frame)) => {
                h2_trace!(?frame, "recv SETTINGS");
                return Ok(ReceivedFrame::Settings(frame));
            }
            Some(GoAway(frame)) => {
                h2_trace!(?frame, "recv GOAWAY");
            ...
            Some(WindowUpdate(frame)) => {
                h2_trace!(?frame, "recv WINDOW_UPDATE");
                self.streams.recv_window_update(frame)?;
            }
            Some(Priority(frame)) => {
                h2_trace!(?frame, "recv PRIORITY");
                // TODO: handle
            }
            None => {
                h2_trace!("codec closed");
