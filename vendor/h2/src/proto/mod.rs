...
// qpx fork: upstream ships a budget of 100 overhead units, which trips
// ENHANCE_YOUR_CALM on legitimate bursts of multiplexed small frames when the
// connection driver drains the socket faster than the consumer dequeues
// events. 4096 units still bounds the DoS exposure while tolerating bursts.
pub const DEFAULT_DATA_FRAME_BUDGET: usize = DEFAULT_DATA_FRAME_OVERHEAD_THRESHOLD * 4096;
