...
#[derive(Debug)]
struct Mutex<T>(ParkingMutex<T>);

#[derive(Debug)]
struct MutexLockError;
...
impl<T> Mutex<T> {
    fn new(value: T) -> Self {
        Self(ParkingMutex::new(value))
    }

    fn lock(&self) -> Result<ParkingMutexGuard<'_, T>, MutexLockError> {
        Ok(self.0.lock())
    }

    fn try_lock(&self) -> Result<ParkingMutexGuard<'_, T>, TryLockError> {
...
impl Inner {
    ...
    fn recv_headers<B>(
        ...
        if stream.state.is_local_error() {
            ...
            h2_trace!("recv_headers; ignoring trailers on {:?}", stream.id);
            return Ok(());
        }
    ...
    B: Buf,
{
    pub fn poll_pending_open(
        ...
        if let Some(pending) = pending {
            ...
            h2_trace!("poll_pending_open; stream = {:?}", stream.is_pending_open);
...
fn drop_stream_ref(inner: &Mutex<Inner>, key: store::Key) {
    let mut me = match inner.lock() {
        ...
        Err(_) => {
            if ::std::thread::panicking() {
                h2_trace!("StreamRef::drop; mutex poisoned");
                return;
            } else {
    ...
    h2_trace!("drop_stream_ref; stream={:?}", stream);
