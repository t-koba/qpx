...
impl Prioritize {
    pub fn new(config: &Config) -> Prioritize {
        ...
        h2_trace!("Prioritize::new; flow={:?}", flow);
    ...
    pub fn queue_frame<B>(
        ...
        let span = h2_trace_span!("Prioritize::queue_frame", ?stream.id);
    ...
    pub fn schedule_send(&mut self, stream: &mut store::Ptr, task: &mut Option<Waker>) {
        ...
        if stream.is_send_ready() {
            h2_trace!(?stream.id, "schedule_send");
            // Queue the stream
        ...
        B: Buf,
    {
        ...
        let span = h2_trace_span!("send_data", sz, requested = stream.requested_send_capacity);
        let _e = h2_span_enter!(span);
        h2_trace!(buffered = stream.buffered_send_data);
    ...
    pub fn clear_pending_capacity(&mut self, store: &mut Store, counts: &mut Counts) {
        let span = h2_trace_span!("clear_pending_capacity");
        let _e = h2_span_enter!(span);
        while let Some(stream) = self.pending_capacity.pop(store) {
            counts.transition(stream, |_, stream| {
                h2_trace!(?stream.id, "clear_pending_capacity");
            })
        }
    }
    ...
    pub fn assign_connection_capacity<R>(
        ...
        R: Resolve,
    {
        let span = h2_trace_span!("assign_connection_capacity", inc);
        let _e = h2_span_enter!(span);
    ...
    fn try_assign_capacity(&mut self, stream: &mut store::Ptr) {
        ...
        let span = h2_trace_span!("try_assign_capacity", ?stream.id);
        let _e = h2_span_enter!(span);
        h2_trace!(
        ...
        if conn_available > 0 {
            ...
            h2_trace!(capacity = assign, "assigning");
    ...
    where
        B: Buf,
    {
        let span = h2_trace_span!("try_reclaim_frame");
        let _e = h2_span_enter!(span);
        ...
        B: Buf,
    {
        ...
        match mem::replace(&mut self.in_flight_data_frame, InFlightData::Nothing) {
            ...
            InFlightData::Drop => {
                h2_trace!("not reclaiming frame for cancelled stream");
                return false;
            }
    ...
    pub fn clear_queue<B>(&mut self, buffer: &mut Buffer<Frame<B>>, stream: &mut store::Ptr) {
        let span = h2_trace_span!("clear_queue", ?stream.id);
        let _e = h2_span_enter!(span);

        // TODO: make this more efficient?
        while let Some(frame) = stream.pending_send.pop_front(buffer) {
            h2_trace!(?frame, "dropping");
        }
    ...
    where
        B: Buf,
    {
        let span = h2_trace_span!("pop_frame");
        let _e = h2_span_enter!(span);

        loop {
            match self.pending_send.pop(store) {
                Some(mut stream) => {
                    let span = h2_trace_span!("popped", ?stream.id, ?stream.state);
                    let _e = h2_span_enter!(span);
                    ...
                    let frame = match stream.pending_send.pop_front(buffer) {
                        Some(Frame::Data(mut frame)) => {
                            ...
                            h2_trace!(len, "sending data frame");

                            // Update the flow control
                            h2_trace_in_scope!(h2_trace_span!("updating stream flow"), {
                            ...
                            let (eos, len) =
                                h2_trace_in_scope!(h2_trace_span!("updating connection flow"), {
                                    // TODO: proper error handling
                        ...
                        None => {
                            if let Some(reason) = stream.state.get_scheduled_reset() {
                                ...
                                h2_trace!("removing dangling stream from pending_send");
                    ...
                    h2_trace!("pop_frame; frame={:?}", frame);
