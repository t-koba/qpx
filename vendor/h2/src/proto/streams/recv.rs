...
impl Recv {
    ...
    pub fn recv_headers(
        ...
        h2_trace!("opening stream; init_window={}", self.init_window_sz);
        ...
        if !pseudo.is_informational() {
            ...
            h2_trace!("Received informational response: stream_id={:?}", stream_id);
        ...
        loop {
            match stream.pending_recv.pop_front(&mut self.buffer) {
                ...
                Some(Event::InformationalHeaders(_)) => {
                    h2_trace!("Skipping informational response in poll_response - should be consumed via poll_informational; stream_id={:?}", stream.id);
                    continue;
                }
    ...
    pub fn release_capacity(
        ...
        h2_trace!("release_capacity; size={}", capacity);
    ...
    pub(crate) fn apply_local_settings(
        ...
        if let Some(target) = settings.initial_window_size() {
            ...
            h2_trace!("update_initial_window_size; new={}; old={}", target, old_sz,);
            ...
            match target.cmp(&old_sz) {
                Ordering::Less => {
                    ...
                    h2_trace!("decrementing all windows; dec={}", dec);
                ...
                Ordering::Greater => {
                    ...
                    h2_trace!("incrementing all windows; inc={}", inc);
                    store.try_for_each(|mut stream| {
    ...
    pub fn enqueue_reset_expiration(&mut self, stream: &mut store::Ptr, counts: &mut Counts) {
        ...
        if counts.can_inc_num_reset_streams() {
            ...
            h2_trace!("enqueue_reset_expiration; added {:?}", stream.id);
    ...
    fn clear_stream_window_update_queue(&mut self, store: &mut Store, counts: &mut Counts) {
        while let Some(stream) = self.pending_window_updates.pop(store) {
            counts.transition(stream, |_, stream| {
                h2_trace!("clear_stream_window_update_queue; stream={:?}", stream.id);
            })
        }
    }
        ...
        B: Buf,
    {
        loop {
            ...
            counts.transition(stream, |_, stream| {
                h2_trace!("pending_window_updates -- pop; stream={:?}", stream.id);
