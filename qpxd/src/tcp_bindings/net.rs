...
pub(crate) fn worker_threads(cfg: &RuntimeConfig) -> usize {
    cfg.worker_threads.unwrap_or_else(default_parallelism)
}

pub(crate) fn max_blocking_threads(cfg: &RuntimeConfig) -> usize {
    cfg.max_blocking_threads
        .unwrap_or_else(|| worker_threads(cfg).saturating_mul(32).max(128))
}

pub(crate) fn acceptor_tasks_per_listener(cfg: &RuntimeConfig) -> usize {
    cfg.acceptor_tasks_per_listener
        .unwrap_or_else(|| worker_threads(cfg))
        .max(1)
}

pub(crate) fn bind_tcp_std_listeners(
    addr: SocketAddr,
...
pub(crate) fn tokio_listener_from_std(listener: StdTcpListener) -> Result<TcpListener> {
    TcpListener::from_std(listener).context("tokio listener conversion failed")
}

#[cfg(feature = "http3")]
pub(crate) fn bind_udp_std_socket(
    addr: SocketAddr,
