...
pub(crate) fn validate_http_status_class(
    status: StatusCode,
    context: &str,
...
pub(crate) fn sanitize_hop_by_hop_headers(headers: &mut HeaderMap, preserve_upgrade: bool) {
...
pub(crate) fn append_via_for_version(headers: &mut HeaderMap, version: Version, proxy_name: &str) {
...
pub(crate) fn sync_host_header_from_absolute_target(headers: &mut HeaderMap, target: &http::Uri) {
...
pub(crate) fn validate_incoming_request<B>(
    req: &http::Request<B>,
...
pub(crate) fn sanitize_response_trailers(trailers: &mut HeaderMap) -> usize {
...
pub(crate) fn validate_h2_h3_connect_headers(
    headers: &HeaderMap,
) -> Result<(), RequestValidationError> {
...
pub(crate) fn validate_h2_h3_request_headers(
    version: Version,
    headers: &HeaderMap,
) -> Result<(), RequestValidationError> {
...
pub(crate) fn normalize_response_for_request<B>(
...
pub(crate) fn normalize_response_for_request_with_options<B>(
    request_method: &Method,
    response: &mut http::Response<B>,
...
pub(crate) fn strip_message_body_headers(headers: &mut HeaderMap) {
    headers.remove(CONTENT_LENGTH);
    headers.remove(TRANSFER_ENCODING);
    headers.remove(TRAILER);
}

pub(crate) fn strip_message_body_framing_headers(headers: &mut HeaderMap) {
    headers.remove(TRANSFER_ENCODING);
    headers.remove(TRAILER);
}

pub(crate) fn sanitize_interim_response_headers(headers: &mut HeaderMap) {
    sanitize_hop_by_hop_headers(headers, false);
    strip_message_body_headers(headers);
}

pub(crate) fn is_hop_by_hop_header_name(name: &str) -> bool {
    matches!(
        name,
        "connection"
            | "proxy-connection"
