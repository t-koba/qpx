...
#[derive(Clone)]
pub(in crate::forward::h3) struct ForwardH3Handler {
    ...
    async fn handle_http(&self, req: Request<Body>, conn: H3ConnInfo) -> Response<Body> {
        ...
        .await
        {
            ...
            Err(err) => {
                ...
                crate::http::protocol::l7::finalize_response_for_request(
                    &request_method,
                    request_version,
    ...
    async fn handle_connect(
        ...
        match kind {
            H3ConnectKind::Connect => {
                let _ = datagrams;
                super::connect::handle_h3_connect(req_head, req_stream, self.clone(), conn).await
            }
            H3ConnectKind::ConnectUdp => {
                super::connect::udp::handle_h3_connect_udp(
                    req_head,
                    req_stream,
                    self.clone(),
                    conn,
            ...
            H3ConnectKind::Extended(protocol) => {
                super::connect::handle_h3_extended_connect(
                    req_head,
                    req_stream,
                    self.clone(),
                    conn,
