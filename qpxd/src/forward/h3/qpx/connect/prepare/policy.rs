...
use crate::policy_context::{
    ...
    enforce_ext_authz, validate_ext_authz_allow_mode,
};
...
fn qpx_connect_entry_scope(
    _protocol: Option<&qpx_h3::Protocol>,
) -> crate::rate_limit::TransportScope {
...
pub(super) async fn apply_connect_rate_limits(
    ...
    let crate::rate_limit::RequestLimitAcquire {
        ...
        None,
        qpx_connect_entry_scope(input.protocol),
        &request_limit_ctx,
        1,
    )?;
...
#[cfg(test)]
mod tests {
    use crate::forward::h3::qpx::connect::prepare::policy::*;

    #[test]
    fn qpx_connect_entry_scope_matches_admission_phase() {
        assert_eq!(
            qpx_connect_entry_scope(None),
            crate::rate_limit::TransportScope::Connect
        );
        assert_eq!(
            qpx_connect_entry_scope(Some(&qpx_h3::Protocol::Other("websocket".to_string()))),
            crate::rate_limit::TransportScope::Connect
        );
        assert_eq!(
            qpx_connect_entry_scope(Some(&qpx_h3::Protocol::ConnectUdp)),
            crate::rate_limit::TransportScope::Connect
        );
    }
}
