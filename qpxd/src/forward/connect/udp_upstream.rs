...
pub(in crate::forward) fn build_upstream_connect_udp_uri(
    upstream: &str,
    target_host: &str,
    target_port: u16,
...
pub(in crate::forward) fn parse_connect_udp_upstream(upstream: &str) -> Result<(String, u16)> {
    if upstream.contains('{') || upstream.contains('}') {
...
#[cfg(test)]
mod tests {
    use crate::forward::connect::udp_upstream::*;
