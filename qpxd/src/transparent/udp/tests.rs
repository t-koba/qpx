...
#[tokio::test]
async fn transparent_session_index_maintenance_paths() {
    let socket = Arc::new(UdpSocket::bind("127.0.0.1:0").await.expect("bind"));
    let (close_tx, _close_rx) = watch::channel(false);
    let client = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 42001);
    let updated = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 42002);
    let permits = AppliedRateLimits::default()
        .acquire_concurrency(&RateLimitContext::default())
        .expect("permits");
    let session = Arc::new(TransparentUdpSession::new(TransparentUdpSessionInit {
        ...
        close_tx,
        client_addr: client,
        target_key: "198.51.100.8:443".to_string(),
        matched_rule: None,
        rate_limit_profile: None,
        seen_ms: 10,
        limits: AppliedRateLimits::default(),
        rate_limit_ctx: RateLimitContext::default(),
        concurrency_permits: permits,
    }));

    let mut index = SessionIndex::new();
    index.insert(7, session.clone());
    assert!(index.session(7).is_some());
    let packet = test_quic_long_header();
    index.observe_client_packet(7, &packet);
    index.observe_upstream_packet(7, &packet);
    index.update_client_address(7, updated);
    assert_eq!(
        index.find_session_for_client_packet(updated, Some("198.51.100.8:443"), &packet),
        Some(7)
    );
    ...
    let permits = AppliedRateLimits::default()
        .acquire_concurrency(&RateLimitContext::default())
        .expect("permits");
    let restored = Arc::new(TransparentUdpSession::new(TransparentUdpSessionInit {
        socket,
        close_tx,
        client_addr: client,
        target_key: "198.51.100.9:443".to_string(),
        matched_rule: None,
        rate_limit_profile: None,
        seen_ms: 20,
        limits: AppliedRateLimits::default(),
    ...
    index.insert_restored(8, restored, Vec::new());
    assert!(index.remove_session(8).is_some());
