...
#[derive(Debug, Clone)]
pub(super) struct SelectedMirrorTarget {
    pub(super) upstream: Arc<UpstreamEndpoint>,
    pub(super) max_mirror_body_bytes: Option<usize>,
}
...
impl ReverseRouter {
    ...
    pub(super) fn new(
        ...
        let runtime_config = qpx_core::config::Config {
            ...
            runtime: qpx_core::config::RuntimeConfig {
                unknown_length_exact_size: qpx_core::config::UnknownLengthExactSizePolicy::Buffer,
                ..Default::default()
            },
    ...
    pub(super) fn new_with_plan(
        ...
        let has_single_direct_local_response_route = matches!(
            http_routes.as_slice(),
            [route] if route.supports_direct_local_response_dispatch()
        );
    ...
    pub(super) fn single_direct_local_response_route(&self) -> Option<&HttpRoute> {
        self.has_single_direct_local_response_route
            .then(|| &self.http_routes[0])
    }

    pub(super) fn single_http_route(&self) -> Option<&HttpRoute> {
        match self.http_routes.as_slice() {
            [route] => Some(route),
            _ => None,
        }
    }
