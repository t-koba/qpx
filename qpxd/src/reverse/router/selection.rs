...
pub(super) fn select_upstream_inner(
    ...
    let rendezvous = |seed: u64| -> Arc<UpstreamEndpoint> {
        ...
        for endpoint in upstreams {
            ...
            if !candidate_in_scope(class, scope) {
                continue;
            }
            let mut hash = seed ^ FNV_OFFSET;
            for b in endpoint.target.as_bytes() {
                hash ^= *b as u64;
    ...
    let selected = match policy.lb {
        LoadBalanceStrategy::RoundRobin => {
            ...
            nth_candidate(upstreams, policy, now_ms, gate_seed, scope, counts, idx)
                .unwrap_or_else(|| upstreams[0].clone())
        }
        LoadBalanceStrategy::Random => {
            let idx = (request_seed as usize) % effective_len;
            nth_candidate(upstreams, policy, now_ms, gate_seed, scope, counts, idx)
                .unwrap_or_else(|| upstreams[0].clone())
        }
        LoadBalanceStrategy::LeastConnections => upstreams
            .iter()
            .filter_map(|endpoint| {
                classify_endpoint(endpoint, policy, now_ms, gate_seed)
                    .filter(|class| candidate_in_scope(*class, scope))
                    .map(|_| endpoint)
            })
            .min_by_key(|endpoint| endpoint.inflight.load(Ordering::Relaxed))
            .cloned()
            .unwrap_or_else(|| upstreams[0].clone()),
...
fn candidate_in_scope(class: EndpointCandidateClass, scope: EndpointSelectionScope) -> bool {
    match scope {
...
fn nth_candidate(
    ...
    match scope {
        EndpointSelectionScope::Eligible => {
            nth_candidate_in_scope(upstreams, policy, now_ms, gate_seed, scope, idx)
        }
        EndpointSelectionScope::AdmittedOnly => {
            nth_candidate_in_scope(upstreams, policy, now_ms, gate_seed, scope, idx)
        }
...
fn nth_candidate_in_scope(
    ...
    let mut seen = 0usize;
    for endpoint in upstreams {
        if let Some(class) = classify_endpoint(endpoint, policy, now_ms, gate_seed)
            && candidate_in_scope(class, scope)
        {
            if seen == idx {
