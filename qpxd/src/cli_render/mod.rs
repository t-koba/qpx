...
pub(crate) use self::match_plan::{MatchPlanRequest, render_match_plan};
...
fn append_execution_details(output: &mut String, plan: &runtime::ExecutionPlan) {
    ...
    if plan.capture.encrypted || plan.capture.plaintext.is_some() {
        ...
        if let Some(plaintext) = plan.capture.plaintext.as_ref() {
            ...
            output.push_str(&format!(
                "      plaintext_body: {}\n",
                plaintext.body.as_str()
            ));
            if let Some(sample_bytes) = plaintext.body_sample_bytes {
                output.push_str(&format!("      body_sample_bytes: {sample_bytes}\n"));
            }
    ...
    if !plan.buffering_reasons.is_empty() {
        output.push_str("    buffering\n");
        output.push_str("      mode: explicit\n");
