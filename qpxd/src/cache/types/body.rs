...
pub struct CachedBodyStream {
    pub(in crate::cache) len: u64,
    pub(in crate::cache) body: Body,
}
...
impl CachedBody {
    pub(in crate::cache) fn from_bytes(bytes: Bytes) -> Self {
        Self::Memory(bytes)
    }

    pub(in crate::cache) fn from_spooled_file(path: PathBuf, len: u64) -> Self {
        Self::File(Arc::new(CachedBodyFile { path, len }))
    }

    pub(in crate::cache) fn len(&self) -> u64 {
        match self {
    ...
    pub(in crate::cache) fn to_body(&self) -> Body {
        match self {
    ...
    pub(in crate::cache) fn slice_to_body(&self, start: u64, end_inclusive: u64) -> Body {
        match self {
            Self::Memory(bytes) => {
    ...
    pub(in crate::cache) async fn from_body_limited(
        mut body: Body,
...
pub(in crate::cache) fn bounded_cache_body_stream(
    mut source: Body,
    max_body_bytes: usize,
    body_read_timeout: Duration,
