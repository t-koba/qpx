...
pub(super) fn encode_command(parts: &[Vec<u8>]) -> Vec<u8> {
...
pub(super) async fn recycle_stream(
    idle: Arc<AsyncMutex<Vec<RedisConnection>>>,
    stream: RedisConnection,
) {
...
pub(super) async fn write_command3(
...
pub(super) async fn read_simple_ok(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
) -> Result<()> {
...
pub(super) async fn read_bulk_len(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
    max_bytes: usize,
) -> Result<Option<usize>> {
...
pub(super) async fn read_bulk_string(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
    max_bytes: usize,
) -> Result<Option<Bytes>> {
...
pub(super) async fn read_bulk_string_array(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
    expected_len: usize,
    max_bytes: usize,
...
pub(super) async fn read_bulk_cached_body(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
    max_bytes: usize,
...
pub(super) async fn stream_redis_bulk_to_body(
    mut conn: RedisConnection,
    timeout_dur: Duration,
    len: usize,
    mut tx: crate::http::body::Sender,
...
pub(super) async fn write_set_cached_body(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
    key: &[u8],
    body: &CachedBody,
...
pub(super) async fn read_exact_buffered(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
    mut dst: &mut [u8],
) -> Result<()> {
...
pub(super) async fn read_integer(conn: &mut RedisConnection, timeout_dur: Duration) -> Result<i64> {
    let line = read_line(conn, timeout_dur).await?;
    if let Some(err) = line.strip_prefix(b"-") {
        return Err(anyhow!("redis error: {}", String::from_utf8_lossy(err)));
    }
...
pub(super) async fn validate_next_append_reply(
    conn: &mut RedisConnection,
    timeout_dur: Duration,
